...
contract LiquidityGauge is ReentrancyGuard {
    ...
    */
    function initialize(address _lpToken) external {
        ...
        string memory _name = string(abi.encodePacked("Opal ", _symbol, " Gauge Deposit"));
        name = _name;
    ...
    */
    ...
    function userCheckpoint(address user) external returns(bool) {
        if (msg.sender != user && msg.sender != MINTER && msg.sender != MINTER_ESCROW) revert CallerNotAllowed();
        _checkpoint(user);
    ...
    function claimableTokens(address user) external returns (uint256) {
        _checkpoint(user);
        return integrateFraction[user] - IMinterEscrow(MINTER_ESCROW).minted(user, address(this));
    }
    ...
    */
    function kick(address user) external {
        ...
        (,,, IVoteLocker.LockedBalance[] memory userLocks) = IVoteLocker(VL_TOKEN).lockedBalances(user);
    ...
    */
    ...
    function permit(
        ...
        bytes32 structHash = keccak256(abi.encode(PERMIT_TYPEHASH, owner, spender, value, userNonce, deadline));
    ...
    */
    ...
    function _checkpoint(address user) internal {
        ...
        uint256 _integrateInvSupplyBoosted = integrateInvSupplyBoosted;
        ...
        integrateInvSupplyBoosted = _integrateInvSupplyBoosted;
